#!/bin/bash
#$ -N subset_GBR
#$ -o logs/subset_GBR_$JOB_ID.out
#$ -e logs/subset_GBR_$JOB_ID.err
#$ -cwd
#$ -l h_rt=2:00:00
#$ -l h_vmem=4G
#$ -l rocky
...
SAMPLE_LIST="GBR_samples.txt"

# Generate GBR_samples.txt if it does not exist.
if [ ! -f "$SAMPLE_LIST" ]; then
  # the panel file has a header and the second column contains population codes.
  awk -F '\t' 'NR > 1 && $2 == "GBR" {print $1}' "$PANEL_FILE" > "$SAMPLE_LIST"
...
for CHR in "${CHROMOSOMES[@]}"; do
  ...
  echo "Subsetting chromosome ${CHR} for GBR samples..."
...
echo "All chromosomes processed for GBR subset!"
